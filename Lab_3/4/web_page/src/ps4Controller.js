...
function canUseGamepadApi() {
  if (!("getGamepads" in navigator)) {
    return "Gamepad API not available in this browser";
  }
  if (!window.isSecureContext) {
    return "Gamepad API blocked: use localhost or HTTPS";
  }
  return "";
}
...
function getGamepads() {
  try {
    ...
    console.warn("Could not read navigator.getGamepads()", error);
    return [];
  }
}
...
export function usePs4Controller({
  ...
  const [controllerStatus, setControllerStatus] = useState(
    "PS4: press a button while the page is open"
  );
  ...
  useEffect(() => {
    ...
    function setConnectionState(isConnected, name = "") {
      ...
      setControllerStatus(
        isConnected
          ? `PS4 connected: ${name || "controller detected"}`
          : "PS4: press a button while the page is open"
      );
    }
